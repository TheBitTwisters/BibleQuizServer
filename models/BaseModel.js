...
const BaseModel = class BaseModel {
  ...
  static search(whereParams, likeParams) {
    return new Promise(async (resolve, reject) => {
      try {
        var q = new mysql.SearchQuery()
        await q.select('*').from(this.tableName).where(whereParams).like(likeParams).sortBy(this.sortBy).execute()
      ...
      try {
        ...
        if (Object.keys(jsonData).length > 0) {
          if (this.id > 0) {
            ...
            await u.update(this.constructor.tableName).set(jsonData).where({ id: this.id }).execute()
            if (u.result) {
              resolve(true)
            }
          } else {
            var i = new mysql.Insert()
            await i.insertInto(this.constructor.tableName).values(jsonData).execute()
            if (i.insertId > 0) {
              this.id = i.insertId
              resolve(true)
            }
